    ...
    ensureAssetConfigHasType: function (assetConfig, cb) {
        ...
        return assetConfig.rawSrcProxy(function (err, rawSrc, metadata) {
            ...
            function foundType(type) {
                if (!type) {
                    that.emit('error', new Error("AssetGraph.ensureAssetConfigHasType: Couldn't determine asset type from asset config: " + util.inspect(assetConfig)) + ", assuming AssetGraph.Asset");
                }
