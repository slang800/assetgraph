...
Css.eachRuleInParseTree = (ruleOrStylesheet, lambda) ->
  _.toArray(ruleOrStylesheet.cssRules).forEach (cssRule) ->
    Css.eachRuleInParseTree cssRule, lambda  if lambda(cssRule, ruleOrStylesheet) isnt false and cssRule.cssRules

Css::eachRuleInParseTree = (lambda) ->
  Css.eachRuleInParseTree @parseTree, lambda
